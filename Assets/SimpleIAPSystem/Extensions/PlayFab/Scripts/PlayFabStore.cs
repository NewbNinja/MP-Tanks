...
namespace SIS
{
    ...
    public class PlayFabStore : IStore
    {
        ...
        /// <summary>
        /// Issues a call to the ConsumeItem API and fires the OnConsumeSucceeded or OnConsumeFailed callbacks.
        /// </summary>
        ...
        /// <summary>
        /// Issues a call to the RedeemCoupon API and handles the outcome by either granting all rewards or fires an error message.
        /// </summary>
        public void RedeemCoupon(string code)
        {
            PlayFabClientAPI.RedeemCoupon(new RedeemCouponRequest() { CouponCode = code }, result =>
            {
                ...
                if (UIShopFeedback.GetInstance() != null && UIShopFeedback.GetInstance().previewWindow != null)
                {
                    ...
                    if (UIShopFeedback.GetInstance().messageWindow != null)
                        UIShopFeedback.GetInstance().messageWindow.gameObject.SetActive(false);

                    UIShopFeedback.ShowPreview(items);
                }
